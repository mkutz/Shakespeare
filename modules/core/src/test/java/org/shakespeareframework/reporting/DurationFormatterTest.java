...
class DurationFormatterTest {
  ...
  void test1(@ForAll @DurationRange Duration duration) {
    assertThat(format(duration)).matches("^((\\d+d)?(\\d+h)?(\\d+m)?(\\d+s)?(\\d+ms)?|<1ms)$");
  }

  @ParameterizedTest(name = "{0}")
  @EnumSource(FormattedDurationExamples.class)
  void test2(FormattedDurationExamples example) {
    assertThat(format(example.duration)).isEqualTo(example.formatted);
  }
  ...
  private enum FormattedDurationExamples {
    ...
    DAYS_32_HOURS_23_MINUTES_59_SECONDS_59_MILLIS_999(
        Duration.ofDays(32).plusHours(23).plusMinutes(59).plusSeconds(59).plusMillis(999),
        "32d23h59m59s");
    ...
    public String toString() {
      return String.format(
          "Duration of %s is formatted to \"%s\"",
          name().toLowerCase().replace('_', ' '), formatted);
    }
  }
}
